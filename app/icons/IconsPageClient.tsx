...
import { iconKeywords } from "../../src/lib/icon-keywords"
import { Navigation } from "../components/Navigation"

// Export format types
type ExportFormat = 'react' | 'vue' | 'svg' | 'figma'

// Icon entry type for search
type IconEntry = {
  name: string
  component: React.ComponentType<{ size?: number; strokeWidth?: number; lively?: MotionType; trigger?: TriggerType; className?: string }>
  keywords: string[]
}
...
const fuseOptions = {
  keys: [
    { name: 'name', weight: 2 },      // Icon name has higher weight
    { name: 'keywords', weight: 1 }   // Keywords have lower weight
  ],
  threshold: 0.35,        // Balance between strict and lenient matching
...
export default function IconsPageClient() {
  ...
  const filteredIcons = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      // Show all icons when no search query
      searchResults = iconEntries
    }
